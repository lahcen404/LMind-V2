...
<div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
    <div>
        <span class="text-[10px] font-black text-lmind-red-mid uppercase tracking-[0.4em] mb-2 block">System Administration</span>
        <h1 class="text-4xl font-black text-slate-900 tracking-tighter uppercase italic">Manage <span class="text-lmind-red-light">Personnel</span></h1>
    </div>

    <a href="{{ route('admin.users.create') }}" class="bg-lmind-navy text-white px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-lmind-red-mid transition-all shadow-xl shadow-slate-900/10 active:scale-95 flex items-center gap-2">
...
<div class="bg-white p-4 rounded-[2rem] shadow-sm border border-slate-100 mb-8 flex flex-col md:flex-row gap-4">
    <div class="flex-1 relative">
        ...
        <input type="text" placeholder="Search by name, email or ID..." class="w-full bg-slate-50 border-none rounded-xl py-3 pl-12 pr-4 text-sm focus:ring-2 focus:ring-lmind-red-light transition">
    </div>
    <select class="bg-slate-50 border-none rounded-xl py-3 px-6 text-sm font-bold text-slate-500 focus:ring-2 focus:ring-lmind-red-light">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="text-[10px] uppercase font-black text-slate-400 bg-slate-50/50 border-b border-slate-50">
                        ...
                        <th class="px-10 py-6">Contact Status</th>
                    ...
                    <tr class="group hover:bg-rose-50/30 transition-colors">
                        <td class="px-10 py-6">
                            <div class="flex items-center gap-4">
                                <div class="w-10 h-10 rounded-xl {{ $user['color'] }} text-white flex items-center justify-center font-black text-xs shadow-inner">
                                    {{ substr($user['name'], 0, 1) }}
                                </div>
                                <div>
                                    <p class="font-black text-slate-800 uppercase tracking-tight">{{ $user['name'] }}</p>
                                    <p class="text-[10px] text-slate-400 font-bold tracking-widest">{{ $user['email'] }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="px-10 py-6">
                        ...
                        <td class="px-10 py-6">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-xs font-bold text-slate-500">{{ $user['status'] }}</span>
                            </div>
                        </td>
                        <td class="px-10 py-6 text-right">
                            <div class="flex justify-end gap-2">
                                <button class="p-2 text-slate-400 hover:text-lmind-red-mid transition" title="Edit User">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                </button>
                                <button class="p-2 text-slate-400 hover:text-lmind-red-dark transition" title="Delete User">
